...
export default defineConfig({
  ...
  // ℹ️ This plugin should be placed before vue plugin
    VueRouter({
      ...
      beforeWriteFiles: root => {
        ...
        // Route détail utilisateur sans crochets dans l'URL (évite "Failed to fetch" avec [id].vue)
        root.insert('/apps/user/view/:id', '/resources/ts/pages/apps/user/view/user-view-by-id.vue')
      },
  ...
  define: { 'process.env': {} },
  server: {
    host: '127.0.0.1', // Avoid [::1] so path segments like [id].vue don't conflict with IPv6 brackets
    proxy: {
      '/api': {
