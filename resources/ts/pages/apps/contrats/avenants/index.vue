...
definePage({ meta: { title: 'Avenants' } })
...
const headers = [
  ...
  { title: 'Type', key: 'type_avenant', sortable: true },
  { title: 'Variation montant', key: 'montant_variation', sortable: false },
  { title: 'Nouveau montant', key: 'nouveau_montant', sortable: true },
  { title: 'Prolongation', key: 'prolongation_jours', sortable: false },
...
const loadData = async () => {
  await store.fetchAvenants({
    contrat_id: filterContratId.value ?? '',
    statut: filterStatut.value,
    date_signature_from: filterDateFrom.value,
    date_signature_to: filterDateTo.value,
...
watch(
  [filterStatut, filterContratId, filterDateFrom, filterDateTo, itemsPerPage, page, sortBy],
  loadData,
  { deep: true },
)
onMounted(loadData)
...
const viewAvenant = (item: any) => router.push(`/apps/contrats/avenants/${item.id}`)
...
const onFormSubmit = async (payload: Record<string, any>) => {
  try {
    ...
    if (selectedItem.value) {
      await store.updateAvenant(selectedItem.value.id, payload)
      snackbar.value = { show: true, text: 'Avenant modifié avec succès', color: 'success' }
    }
    else {
      await store.createAvenant(cid, payload)
      snackbar.value = { show: true, text: 'Avenant créé avec succès', color: 'success' }
    }
...
const confirmDelete = async () => {
  ...
  try {
    await store.deleteAvenant(selectedItem.value.id)
    deleteDialog.value = false
    snackbar.value = { show: true, text: 'Avenant supprimé avec succès', color: 'success' }
    await loadData()
  }
  catch {
    snackbar.value = { show: true, text: 'Impossible de supprimer cet avenant', color: 'error' }
  }
}

const doAction = async (action: 'submitAvenant' | 'approveAvenant', item: any) => {
  try {
    await store[action](item.id)
    snackbar.value = {
      show: true,
      text: action === 'submitAvenant' ? 'Avenant soumis' : 'Avenant approuvé',
      color: 'success',
    }
    await loadData()
  }
...
const confirmReject = async () => {
  if (!selectedItem.value) return
  try {
    await store.rejectAvenant(selectedItem.value.id, commentaireRejet.value)
    rejectDialog.value = false
    snackbar.value = { show: true, text: 'Avenant rejeté', color: 'warning' }
    await loadData()
  }
  catch {
...
<template>
      ...
      <VCard>
        ...
        <VCardText>
          <VRow class="mb-3">
            ...
            <VCol cols="12" md="2">
              <VTextField v-model="filterDateFrom" label="Date signature du" type="date" density="compact" />
            </VCol>
            <VCol cols="12" md="2">
          ...
          >
            ...
            <template #item.type_avenant="{ item }">
              {{ typeLabels[item.type_avenant] ?? item.type_avenant }}
            </template>
            <template #item.montant_variation="{ item }">
              <span v-if="item.montant_variation != null" :class="item.montant_variation >= 0 ? 'text-success' : 'text-error'">
                {{ formatMontant(item.montant_variation) }}
              </span>
              <span v-else>-</span>
            </template>
            <template #item.nouveau_montant="{ item }">
              <span class="font-weight-medium">{{ formatMontant(item.nouveau_montant) }}</span>
            </template>
            <template #item.prolongation_jours="{ item }">
              {{ item.prolongation_jours != null ? `${item.prolongation_jours} j` : '-' }}
            </template>
            ...
            <template #item.actions="{ item }">
              <VBtn icon variant="text" size="small" color="secondary" @click="viewAvenant(item)">
              ...
              <VBtn v-if="item.statut === 'draft'" icon variant="text" size="small" color="info" @click="doAction('submitAvenant', item)">
                <VIcon icon="tabler-send" />
                <VTooltip activator="parent">Soumettre</VTooltip>
              </VBtn>
              <VBtn v-if="item.statut === 'submitted'" icon variant="text" size="small" color="success" @click="doAction('approveAvenant', item)">
  ...
  <VDialog v-model="formDialog" max-width="800" scrollable persistent>
    <VCard :title="selectedItem ? 'Modifier l\'avenant' : 'Nouvel avenant'">
      <VCardText>
        <VSelect
          v-if="!selectedItem"
  ...
  <VDialog v-model="rejectDialog" max-width="500">
    <VCard>
      ...
      <VCardText>
        <p class="mb-3">
          Avenant <strong>{{ selectedItem?.numero }}</strong> — {{ selectedItem?.contrat?.reference }}
        </p>
  ...
  <VDialog v-model="deleteDialog" max-width="420">
    <VCard>
      ...
      <VCardText>
        Voulez-vous vraiment supprimer l'avenant <strong>{{ selectedItem?.numero }}</strong> ?
      </VCardText>
      <VCardActions class="justify-end pa-4">
