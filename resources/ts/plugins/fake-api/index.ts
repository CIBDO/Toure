// MSW (Mock Service Worker) est désactivé car nous utilisons le vrai backend Laravel
// Pour réactiver MSW, décommentez le code ci-dessous et ajoutez VITE_USE_MSW=true dans .env
...
export default function () {
  // MSW est désactivé - nous utilisons le vrai backend Laravel
  // Pour réactiver MSW, décommentez le code ci-dessus et ajoutez VITE_USE_MSW=true dans .env
  ...
  if (import.meta.env.DEV && import.meta.env.VITE_USE_MSW === 'true') {
    ...
    try {
      worker.start({
        ...
        console.warn('MSW n\'a pas pu démarrer:', error)
      })
    } catch (error) {
      console.warn('MSW n\'a pas pu être initialisé:', error)
    }
  }
  */
}
